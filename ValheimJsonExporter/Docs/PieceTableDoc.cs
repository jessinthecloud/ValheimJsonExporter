...
namespace ValheimJsonExporter.Docs
{
    public class PieceTableDoc : Doc
    {
        ...
        public PieceTableDoc() : base("piece-table-list.json")
        {
            PieceManager.OnPiecesRegistered += docPieceTables;
        }

        public PieceTableDoc(string file) : base(file)
        {
            PieceManager.OnPiecesRegistered += docCSPieceTables;
        }
        ...
        public void docCSPieceTables()
        {
            ...
            foreach (var pair in pieceTables)
            {
                ...
                jsonPieceTable.Add("name", pair.Key); 
                jsonPieceTable.Add("jotunn_alias", alias); 
                jsonPieceTable.Add("num_pieces", pair.Value.m_pieces.Count.ToString());
